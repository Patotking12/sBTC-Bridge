...
####################################################################################################################################################################################
    ...
    def generate_script(self):
        print("Generating Script and fetching all necessary Data")
       ...
       # Fetch and display the sBTC wallet address
        ...
        # Automatically populate the Recipient Address field
        # This will be generated by the script with the endpoint for the time being it will be this address
        self.address_input.setText("tb1qdxuhyz6vql80zt4ef94wg3f02dyxra38lcecng")

    def fetch_and_store_keys(self):
        print("Fetching Keys and generating Data")
        try:
            url = "https://testnet.stx.eco/bridge-api/testnet/v1/btc/tx/keys"
            response = requests.get(url)
            data = response.json()
            reveal_pub_key = data['deposits']['revealPubKey']
            print("Reveal Pub Key:", reveal_pub_key)
            reclaim_pub_key = data['deposits']['reclaimPubKey']
            print("Reclaim Pub Key:", reclaim_pub_key)
    ...
    def fetch_sbtc_wallet_address(self):
        print("Fetching sBTC Wallet Address")
        try:
            response = requests.get('https://testnet.stx.eco/bridge-api/testnet/v1/sbtc/data')
            data = response.json()
            wallet_address_general = data.get('sbtcWalletAddressInfo')
            wallet_address = wallet_address_general.get('address')
            print("sBTC Wallet:", wallet_address)
    ...
    def fetch_btc_wallet_address(self, window):
        print("Fetching BTC Wallet with highest BTC amount")
        try:
            ...
            print("Wallet with highest balance:", address_with_max_balance)
    ...
    def send_btc(self, window, password):
        ...
        try:
            ...
            print("BTC Amount:", amount_btc)  # Get the amount of bitcoin to send
            recipient_address = self.address_input.text()
            print("Recipient:", recipient_address)  # Get the recipient's address
            fee_option = self.fee_combo.currentText()
            print("fee option:", fee_option)

            print("Extracting fee amount...")
            # Extract the fee amount from the selected option
            fee_btc = float(fee_option.split(":")[0].strip().split(" ")[0])
            print("fee btc:", fee_btc)
            ...
            print("Amount to send in Sats", amount_sat)  # Convert the amount to satoshis
            wallet = window.wallet 
            print("Wallet:", wallet) # Get the wallet object
            ...
            QMessageBox.information(None, _('Transaction Success'), _('Tx Success! Please check History to view your transaction, and add your STX address to the peg in history to view the status'))
...
####################################################################################################################################################################################
    ...
    def create_tx_history_peg_in_tab(self, window):
        ...
        reclaim_deposit_button = EnterButton(_('Reclaim BTC'), lambda: self.reclaim_deposit_password(window))
        reclaim_deposit_button.setMaximumWidth(150)  # Set the maximum width for the button
        vbox.addWidget(reclaim_deposit_button)      
    ...
    def add_address_tx_history_peg_in(self):
        print("Adding address to track peg in Transactions")
        address_tx_history_peg_in, ok = QInputDialog.getText(self, "Add Address", "Enter an address:")
        print("Address added:", address_tx_history_peg_in)
        if ok:
    ...
    def fetch_tx_history_peg_in(self, address):
        print("Fetching all peg in transactions of", address)
    ...
    def refresh_tx_history_peg_in(self):
        for row in range(self.tx_history_peg_in_table.rowCount()):
            address = self.tx_history_peg_in_table.item(row, 1).text()
            self.fetch_tx_history_peg_in(address)
    
    def adjust_column_widths(self):
        self.tx_history_peg_in_table.resizeColumnsToContents()

    def reclaim_deposit_password(self, window):
        # Prompt the user for the ID
        id, ok = QInputDialog.getText(window, _('Enter ID'), _('Please enter the ID'), QLineEdit.Normal)
        if not ok:
            return  # User canceled, return early

        # Prompt the user for the password
        password, ok = QInputDialog.getText(window, _('Enter Password'), _('Please enter your password'), QLineEdit.Password)
        if ok:
            self.reclaim_deposit(window, id, password)

    def reclaim_deposit(self, window, id, password):
        print("Reclaiming Deposit id:", id)
        # Handle reclaiming of deposit here
        QMessageBox.information(window, "Transaction Success", "Tx Success! Track the status in the peg in history tab by adding your STX address")
...
####################################################################################################################################################################################
    ...
    def prompt_password_withdraw(self, window):
        password, ok = QInputDialog.getText(window, "Password", "Enter your password:", QLineEdit.Password)
        if ok:
            self.withdraw_sbtc(window, password)

    def withdraw_sbtc(self, window, password):
        print("Withdrawing BTC")
        QMessageBox.information(window, "Transaction Success", "Tx Success! Track your transaction in the peg out history tab by adding your STX address")
...
####################################################################################################################################################################################
    ...
    def create_tx_history_peg_out_tab(self, window):
        ...
        reclaim_withdraw_button = EnterButton(_('Reclaim sBTC'), lambda: self.reclaim_withdrawal_password(window))
        reclaim_withdraw_button.setMaximumWidth(150)  # Set the maximum width for the button
        vbox.addWidget(reclaim_withdraw_button)
    ...
    def add_address_tx_history_peg_out(self):
        print("Adding address to track peg out Transactions")
        address_tx_history_peg_out, ok = QInputDialog.getText(self, "Add Address", "Enter an address:")
        print("Address added:", address_tx_history_peg_out)
        if ok:
    ...
    def fetch_tx_history_peg_out(self, address_tx_history_peg_out):
        print("Fetching all peg in transactions of", address_tx_history_peg_out)
    ...
    def refresh_tx_history_peg_out(self):
        for row in range(self.tx_history_peg_out_table.rowCount()):
            address = self.tx_history_peg_out_table.item(row, 1).text()
    ...
    def reclaim_withdrawal_password(self, window):
        # Prompt the user for the ID
        id, ok = QInputDialog.getText(window, _('Enter ID'), _('Please enter the ID'), QLineEdit.Normal)
        if not ok:
            return  # User canceled, return early
        ...
        password, ok = QInputDialog.getText(window, _('Enter Password'), _('Please enter your password'), QLineEdit.Password)
        if ok:
            self.reclaim_withdrawal(window, id, password)

    def reclaim_withdrawal(self, window, id, password):
        print("Reclaiming Withdrawal id:", id)
        # Handle reclaiming of withdrawal here
        QMessageBox.information(window, "Transaction Success", "Tx Success! Track the status in the peg out history tab by adding your STX address")
...
####################################################################################################################################################################################
    ...
    def add_address(self):
        print("Adding address to track balance")
    ...
    def update_balance(self, address, stx_balance, sbtc_balance):
        ...
        for i in range(self.summary_table.rowCount()):
            if self.summary_table.item(i, 0).text() == address:
                ...
                print("STX:", stx_balance_item, "in", address)
                sbtc_balance_item = QTableWidgetItem(str(sbtc_balance))
                print("sBTC:", sbtc_balance_item, "in", address)
    ...
    def remove_address(self):
        print("Removing selected address")
    ...
    def refresh_addresses(self):
        ...
        for row in range(self.summary_table.rowCount()):
            print(range(self.summary_table.rowCount()))
